...
def load_symbols_from_file(path: str | Path, symbol_column: str | None = None) -> list[str]:
    ...
    TXT files should contain one symbol per line. Blank lines and lines starting
    with ``#`` are ignored. CSV files use ``symbol_column`` when provided; otherwise
    the first column is treated as the ticker column.
    """
...
def scan_results_to_dataframe(results: list[ScanResult]) -> pd.DataFrame:
    ...
    for result in results:
        ...
        weekly_signals = ",".join(result.timeframe_results["weekly"]["triggered_signals"]) or "None"
        daily_signals = ",".join(result.timeframe_results["daily"]["triggered_signals"]) or "None"
        hour4_signals = ",".join(result.timeframe_results["4hour"]["triggered_signals"]) or "None"

        weekly_factors = ",".join(result.timeframe_results["weekly"]["triggered_factors"]) or "None"
        daily_factors = ",".join(result.timeframe_results["daily"]["triggered_factors"]) or "None"
        hour4_factors = ",".join(result.timeframe_results["4hour"]["triggered_factors"]) or "None"
        ...
        row = {
            ...
            "weekly_confluence_tier": result.timeframe_results["weekly"]["factor_confluence_tier"] or "None",
            "weekly_confluence_score": result.timeframe_results["weekly"]["factor_confluence_score"],
            "daily_triggered_signals": daily_signals,
            "daily_triggered_factors": daily_factors,
            "daily_confluence_tier": result.timeframe_results["daily"]["factor_confluence_tier"] or "None",
            "daily_confluence_score": result.timeframe_results["daily"]["factor_confluence_score"],
            "4hour_triggered_signals": hour4_signals,
            "4hour_triggered_factors": hour4_factors,
            "4hour_confluence_tier": result.timeframe_results["4hour"]["factor_confluence_tier"] or "None",
            "4hour_confluence_score": result.timeframe_results["4hour"]["factor_confluence_score"],
